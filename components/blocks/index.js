...
import TwoColumnText from './organisms/TwoColumnText';

const renderBlock = ({ type, ...props }, i) => {
  const key = `${type}_${props.name}`;
  let Block = (
    <div className="container mx-auto mt-2 mb-2 text-center">
  ...
  switch (type) {
    ...
    case 'TeamRoster':
      ...
      break;
    case 'TwoColumnText':
      Block = <TwoColumnText {...props} key={key} />;
      break;
  }
  return Block;
};
...
const BlockRenderer = ({ organisms }) =>
  organisms ? organisms.map(o => renderBlock(o)) : null;
